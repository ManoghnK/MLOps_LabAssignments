...
## UI and recent changes

This repository now includes a small single-page UI that calls the existing `/predict` endpoint and is served from the FastAPI app root (`/`). The UI is intentionally lightweight and uses a 21st.dev-inspired visual style. Key points:

- UI root: `/` — serves `src/static/index.html` (form + client JS).
- Static assets: `src/static/styles.css`, `src/static/app.js`.
- Health endpoint moved to `/health` (root `/` is the UI).

Visuals (screenshots):

![Landing Page](Screenshot_of_UI.png)

![On Running with results](Screenshot_of_UI_After_running.png)

Quick run (if you followed the original setup):
...
Files changed/added for the UI
- `src/main.py` — mounts the `static` directory and serves `index.html` at `/` while keeping `/predict` and `/health`.
- `src/static/index.html` — the new single-page UI.
- `src/static/app.js` — client logic posting to `/predict`.
- `src/static/styles.css` — theme / colors (edit this file to change colors).

Quick tests

- Health: `curl http://127.0.0.1:8000/health`
- Predict: use the UI or run:
...
If you'd like, I can:
- map numeric class ids to species names in the UI,
- add prediction probabilities, or
...
└── fastapi_lab1
    ├── assets/
    ├── fastapi_lab1_env/     # optional: a provided virtualenv for convenience
    ├── model/
    │   └── iris_model.pkl
    ├── src/
    │   ├── __init__.py
...
![API page](./Screenshot of UI.png)
![API response](./Screenshot of UI After running.png)
