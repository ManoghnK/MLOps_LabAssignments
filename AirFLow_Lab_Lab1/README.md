...
## My changes to the flow

The following updates were made to the Lab_1 codebase to support faster local testing and clearer logs:

- Reduced the KMeans search range in `dags/src/lab.py` from `range(1, 50)` to `range(1, 11)` to speed up model-building iterations during development.
- Added debug print statements inside `dags/src/lab.py` (in `load_data`, `data_preprocessing`, and `build_save_model`) so task logs contain small, informative messages (data shapes, sample rows, saved model path and last k). These prints will appear in Airflow task logs when tasks run.
- Appended a test row to `dags/data/test.csv` (header plus one example record: `BALANCE,PURCHASES,CREDIT_LIMIT` -> `1200,300,3000`) so the `load_model_elbow` step has a sample record to predict against during tests.

Also: a new admin user was added via the Airflow UI (Users → Create / List Users) to facilitate quick UI access and verification of permissions and DAG control — verify the user under `Security > Users` in the Airflow web UI. 

How to verify these changes quickly:

- Start the stack: `docker compose up -d --build`
- Open Airflow UI at `http://localhost:8080` and confirm the new user under `Security > Users`.
- Unpause `Airflow_Lab1` and trigger a DAG run; inspect task logs for the debug prints and the elbow output.
